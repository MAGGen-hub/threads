	...
	try{
		...
		#pragma omp parallel num_threads(numberOfThreads)
		{
			...
			for (int i = 0; i < n; i++)//files init
			{
				...
				char buffer[100];
				string str = fileway;
				_itoa(i + 1, buffer, 10);//возможны проблемы при компиляции с помощью с++ Source.cpp -fopenmp //при возникновении заменить на _itoa_s(i + 1, buffer, 10);
				str.append(buffer);
		...
		#pragma omp parallel
		{
			do
			{
				...
				for (int j = 0; j < n; j++)
				{
					_itoa(num[j], buffer, 10);//возможны проблемы при компиляции с помощью с++ Source.cpp -fopenmp //при возникновении заменить на _itoa_s(num[j], buffer, 10);
